...
#
# all these are in the executable...
# --> $source setup_env.sh
#
...
#######################################
	...
	while (len(inputfiles) > 0):
		...
		for line in range(min(opt.nfileperjob,len(inputfiles))):
			...
			if(opt.eos!=''):	
				outputfile.write('cp '+rootoutputfile+'_tree.root /eos/user/c/cbasile/HLTapplyDATA/'+output+'_'+str(ijob)+'_tree.root\n')
				outputfile.write('cp '+rootoutputfile+'_histo.root /eos/user/c/cbasile/HLTapplyDATA/'+output+'_'+str(ijob)+'_histo.root\n')
